...
import { PlanejamentoContainer } from '../Planejamento/styles';
...
interface ReuniaoPageProps {
    acoes: React.ReactNode;
    atores: React.ReactNode;
}

export default function ReuniaoPage({ acoes, atores }: Readonly<ReuniaoPageProps>) {
    const { caso } = useCasoSelecionado();
    const { dataReuniao } = useParams();
    const { register, handleSubmit } = useForm<DataReuniaoFormField>({});

    const [errorData, setErrorData] = useState<string | null>(null);
    const [, setData] = useState<Date | null>(null);
    ...
    const handleSubmitDataReuniao = useCallback(
        async (formData: DataReuniaoFormField) => {
            ...
            try {
                const reqData: AgendarReuniaoRequest = { data: dataFormatada };
                setData(dataSelecionada);
                setErrorData(null);
                await agendarReuniao(reqData, caso.id);
                Swal.fire({
                    title: 'Reunião agendada!',
                    text: `A reunião foi marcada para ${dataSelecionada.toLocaleString()}.`,
                    icon: 'success',
                    timer: 4000,
    ...
    return (
        <PlanejamentoContainer>
            ...
            <div className="row">
                <div className="column">
                    <BoxContainer titulo={''}>
                        ...
                        <textarea
                            ...
                            onFocus={(e) => (e.currentTarget.style.border = '1px solid #134780')}
                ...
                <div className="column">
                    <div style={{ marginBottom: '20px' }}>{atores}</div>
